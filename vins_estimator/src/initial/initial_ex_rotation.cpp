...
/*(cxn)
注意，这段代码只标定了旋转
找到连续两帧图片的共同特征点，通过对极几何，可以计算出连续两帧图片的旋转R_{cicj}
通过预积分可以得到这两帧图片的旋转R_{bibj}
满足 R_{bibj}*R_{bc}=R_{bc}*R_{cicj}
通过 四元数的左乘右乘运算，可得  Q^i_j * R_{bc}=0
最小二乘问题，SVD分解即可，在凑够10帧且SVD分解出的倒数第二个奇异值大于0.25之后，这个过程会一直搜集上述的等式，并持续更新 R_{bc}
另外，作者还特别讲究地加了一个核函数：若 R_{cb}*R_{bjbi}*R_{bc}*R_{cicj} 对应的角度小于5°，则会抑制其对应的 Q^i_j
*/
