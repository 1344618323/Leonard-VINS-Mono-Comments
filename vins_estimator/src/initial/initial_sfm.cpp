 ...
 * (cxn)粗糙地说下过程：之前已经得到了l帧到当前帧的变换T（l>=0&&l<frame_count,当前帧就是frame_count，也即是10）
 * 通过这个T，我们能将 l帧 与 frame_count帧 的共视特征点 三角化， 其中l帧位姿初始化为E
 * 再开始从 j = l+1 -> frame_count-1 之间循环：
 * 		找到j帧 中 l帧看到的且三角化了 的特征点， 通过pnp 得到这些帧的 初始 Tj
 * 		pnp后，再查找 j帧 与 l帧 的 共视且未三角化特征点，三角化
 * j = l-1 -> 0 帧 也是同理
 * 然后，再遍历窗口内所有特征点，将未三角化的都三角化
 * 最后，BA优化，窗口内所有相机位姿、所有特征点3D坐标（其中l帧的R、t固定；frame_count帧t固定：为了固定尺度）
*/
