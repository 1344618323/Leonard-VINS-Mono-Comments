...
void solveGyroscopeBias(map<double, ImageFrame> &all_image_frame, Vector3d* Bgs)
{
    ...
    /*(cxn)
    我们知道误差传递式： δ(x_{k}) = F δ(x_{0}) + G δ(u_{0})
                      x_{k} = x^{hat}_{k} + δ(x_{k}) =  x^{hat}_{k} + F δ(x_{0}) + G δ(u_{0})
    x^{hat}_{k}指真实值
    x_{k}指通过传感器获得的值

    我们通过预积分可以获得一帧图片的 q_{bkbk+1}，对其做关于 δ(b^g)的一阶泰勒展开，q_{bkbk+1}= q^{hat}_{bkbk+1} * [1; 1/2* δ(theta)] 
                                                                            = q^{hat}_{bkbk+1} * [1, 1/2* J^{theta}_{b^g}* δ(b^g)] 
    通过相机的pnp与三角化，以及相机IMU外参，可得 q^{c0}_{bk}、q^{c0}_{bk+1}
    满足： || (q^{c0}_{bk})^{-1} * q^{c0}_{bk+1} * q^{hat}_{bkbk+1} ||_{xyz} = 0


    我们目标是求 min_{δ(b^g} (||2 (q^{c0}_{bk})^{-1} * q^{c0}_{bk+1} * q_{bkbk+1}).vec() ||^2)
    其中， q_{bkbk+1} = q^{hat}_{bkbk+1} * [1, 1/2* J^{theta}_{b^g}* δ(b^g)] 
    高斯牛顿法，其实就是求方程 ： sum{J^{theta}_{b^g}} δ(b^g} =  sum {-2 (q^{c0}_{bk})^{-1} * q^{c0}_{bk+1} * q_{bkbk+1}).vec()}
    */
