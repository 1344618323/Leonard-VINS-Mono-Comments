...
void predict(const sensor_msgs::ImuConstPtr &imu_msg)
{
    ...
    if (init_imu)
    {
        ...
        init_imu = 0;
        return;
        //(cxn) 第一帧的时候  q_{wbk+1} p_{wbk+1} v^w_{k+1} 输出是 0
        // 目测是有bug，应该加上
    ...
    /*(cxn)
     *  un_acc_0 = q_{wbk} ( a^{bk} - b^a_k ) - g^w
     *  un_gyr =  w = 1/2 ( w^{bk} + w^{bk+1}  ) - b^g_k
     *  tmp_Q = q_{wbk+1} = q_{wbk} [1 ; 1/2*w*dt]
     *  un_acc_1 = q_{wbk+1} ( a^{bk+1} - b^a_k ) - g^w
     *  un_acc = a = 1/2 (un_acc_1 + un_acc_0)
     *  tmp_P = p_{wbk+1} = p_{wbk} + v^w_k*dt + 1/2*a*dt*dt
     *  tmp_V = v^w_{k+1} = v^w_k + a*dt
     */
    ...
    while (true)
    {
        ...
        for (auto &measurement : measurements)
        {
            ...
            for (auto &imu_msg : measurement.first)
            {
                ...
                /*(cxn)
                梳理一下 IMU数据的处理方式
                IMU数据时间戳     I0     I1     I2     I3     I4     I5      I6
                gyro数据时间戳  M0                  M1                    M2
                其中   I0 I1 I2 I3 M1 打包， I3 I4 I5 I6 M2 打包
                对于第一组数据，积累 从 M0时刻到M1时刻内的IMU数据
                这段时间内的预积分可以不严谨地表示为：Imu_I0(T_I0- T_M0) + Imu_I1(T_I1- T_I0) +Imu_I2(T_I2- T_I1) + Imu_M1(T_M1-T_I2)
                其中Imu_M1通过 Imu_I2、Imu_I3 线性插值得到
                注意这个时间段用的预积分对象（是一个IntegrationBase类），其acc_0，gyr_0是Imu_M0
                对于第二组数据同理
                */
                if (t <= img_t)
                { 
