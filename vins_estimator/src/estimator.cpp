...
bool Estimator::initialStructure()
{
    ...
    for (int i = 0; frame_it != all_image_frame.end( ); frame_it++)
    {
        ...
        if((frame_it->first) == Headers[i].stamp.toSec())
        {
            ...
            //(cxn) R^{c_{ref}}_{c_k}* (R^b_{c_{ref}})^{-1} =R^{c_{ref}}_{b_k}
            //      t^{c_{ref}}_{c_k}
        ...
        //(cxn)这里先将该帧后第一个关键帧的位姿作为该帧的位姿初始估计，后面solvepnp的时候会优化
...
bool Estimator::visualInitialAlign()
{
    ...
    for (int i = 0; i <= frame_count; i++)
    {
        ...
        //(cxn) t^{c_{ref}}_{c_k}
        //      R^{c_{ref}}_{c_k}* (R^b_{c_{ref}})^{-1} =R^{c_{ref}}_{b_k}
        Ps[i] = Pi;
        Rs[i] = Ri;
    ...
    for (int i = frame_count; i >= 0; i--)
        ...
        //(cxn) 原来是 Ps[i] = t^{c_{ref}}_{c_k}，变成 Ps[i] = t^{c_{ref}}_{b_0 b_k}
        //      注意此时，Ps[0] = [0; 0; 0]
    ...
    for (frame_i = all_image_frame.begin(); frame_i != all_image_frame.end(); frame_i++)
    {
        if(frame_i->second.is_key_frame)
        {
            kv++;
            //Vs为优化得到的速度
            //(cxn) v^{c_{ref}}_{b_k} = R^{c_{ref}}_{b_k} * v^{b_k}_{b_k}
    ...
    //(cxn) 将 R^w_{b_0}的yaw角去了，也就是说world系的设定是：第一帧的body系 在 world系中的 yaw=0，x=y=z=0
    ...
    for (int i = 0; i <= frame_count; i++)
    {
        Ps[i] = rot_diff * Ps[i];//(cxn)获取 t^w_{b_k}，自此完成了重力向量对齐
        Rs[i] = rot_diff * Rs[i];//         R^w_{b_k}
        Vs[i] = rot_diff * Vs[i];//         v^w_{b_k} 
    }
...
void Estimator::solveOdometry()
{
    ...
    {
        TicToc t_tri;
        //(cxn)对特征点重新三角化一次（之前的三角化只用了两帧的观测，而在这个函数中，对每一个特征点的三角化，每一个观测都会用到）
...
void Estimator::double2vector()
{
    ...
    // 求得优化前后的姿态差
    /*(cxn)
    计算优化后的bn系 yaw 角 相对于 原来的bo系 yaw 角 在 world系下 差了多少 deltaY，
    意思就是说 在世界系下， bn系 需要沿着 世界系 的z轴再转 deltaY 才能得到 bo系
    优化后坐标系旋转为 R^w_{bn} ，优化前坐标系旋转为  R^w_{bo}
    坐标系旋转，左乘： R^w_{bo} = R(deltaY) * R^w_{bn} 
    坐标系位移也做旋转： R(deltaY)*(t^w_{bi}-t^w_{bn})+ t^w_{bo}
    */
    ...
    { 
        ...
        //(cxn)这里我们得到，vio优化后的回环帧位姿与在posegraph中位姿的差
        drift_correct_yaw = Utility::R2ypr(prev_relo_r).x() - Utility::R2ypr(relo_r).x();
        drift_correct_r = Utility::ypr2R(Vector3d(drift_correct_yaw, 0, 0));
        drift_correct_t = prev_relo_t - drift_correct_r * relo_t;   
        
        //(cxn)得到与回环帧共视的滑窗中帧 在posegraph坐标系中 的位姿差
...
void Estimator::optimization()
{
    ...
    loss_function = new ceres::CauchyLoss(1.0);//(cxn) 1/2 \sum_i r^2_i， 加入核函数后， 1/2 \sum c^2 log(1+(r^2_i/c))
        ...
        if (last_marginalization_info)
        {
            ...
            for (int i = 0; i < static_cast<int>(last_marginalization_parameter_blocks.size()); i++)
            {
                if (last_marginalization_parameter_blocks[i] == para_Pose[0] ||
                    ...
                    //(cxn)若上一次保留下来的参数块中有这次要边缘化的参数块，记录下来
...
void Estimator::slideWindow()
{
    ...
    else
    {
        if (frame_count == WINDOW_SIZE)
        {
            //(cxn)若次新帧不是关键帧，将次新帧带来的特征点、预积分、位姿去掉，同时用次新帧到最新帧率的IMU数据 加入到 次次新帧到次新帧的预积分，从而得到次次新帧到最新帧的预积分
