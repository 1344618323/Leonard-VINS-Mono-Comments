...
struct ResidualBlockInfo
{
    ...
    ceres::CostFunction *cost_function;//(cxn) IMU提供的cost_function的尺寸 <15,7,9,7,9>
    ceres::LossFunction *loss_function;
    std::vector<double *> parameter_blocks;//优化变量数据 (cxn) IMU提供的参数块大小为7,9,7,9
    std::vector<int> drop_set;//待边缘化的优化变量id       (cxn) 要边缘化的参数块号（0,1）,也就是前面的 7,9
