                         ...
                         Eigen::Vector3d &PnP_T_old, Eigen::Matrix3d &PnP_R_old)
{
    ...
    Matrix3d R_inital;
    Vector3d P_inital;
    Matrix3d R_w_c = origin_vio_R * qic;//(cxn)R^w2_jc
    Vector3d T_w_c = origin_vio_T + origin_vio_R * tic;//(cxn)t^w2_jc

    R_inital = R_w_c.inverse();//(cxn)R^j_w2，在pnp中我们使用T^j_w2作为待求的T^i_w2的初值
    P_inital = -(R_inital * T_w_c);//(cxn)t^j_w2
    ...
    cv::Rodrigues(rvec, r);//(cxn)我们将此时的vio的世界坐标系设置为w2，有R^ic_w2
    Matrix3d R_pnp, R_w_c_old;
    cv::cv2eigen(r, R_pnp);
    R_w_c_old = R_pnp.transpose();//(cxn)R^w2_ic
    Vector3d T_pnp, T_w_c_old;
    cv::cv2eigen(t, T_pnp);
    T_w_c_old = R_w_c_old * (-T_pnp);//(cxn)t^w2_ic

    PnP_R_old = R_w_c_old * qic.transpose();//(cxn)R^w2_i
    PnP_T_old = T_w_c_old - PnP_R_old * tic;//(cxn)t^w2_i
...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
	...
	*/
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
		//(cxn) t^i_j = R^i_w2 * (t^w2_j - t^w2_i)
	    relative_t = PnP_R_old.transpose() * (origin_vio_T - PnP_T_old);
		//(cxn) R^i_j = R^i_w2 * R^w2_j
