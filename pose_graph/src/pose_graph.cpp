...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
	...
	if (loop_index != -1)
	{
        ...
        if (cur_kf->findConnection(old_kf))
        {
            ...
            //(cxn)设当前帧为j，回环帧为i，这里得到了 T^i_j；我们认为i帧的世界坐标系为w1，而j帧的世界坐标系为w2
            relative_t = cur_kf->getLoopRelativeT();
            relative_q = (cur_kf->getLoopRelativeQ()).toRotationMatrix();

            //重新计算当前帧位姿w_P_cur、w_R_cur
            //(cxn)得到 T^w1_i*T^i_j = T^w1_j 
            ...
            Matrix3d shift_r;
            Vector3d shift_t; 
            //(cxn) shift_r = yaw(T^w1_j) - yaw(T^w2_j)
            //      shift_t = t^w1_j - R^w1_j R^j_w2 t^w2_j
        ...
        if (cur_index != -1)
        {
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                ...
                for (int j = 1; j < 5; j++)
                {
                  ...
                  {
                    //(cxn) euler_conncected = R^w1_{i-j}
                    //      relative_t = R^{i-j}_w1 (t^w1_i - t^w2_{i-j})
                    //      relative_yaw = yaw(i) - yaw(i-j)
                ...
                if((*it)->has_loop)
                {
                    ...
                    //(cxn)为了方便表示，我们依然设置old为回环帧，cur为当前帧，之前在闭环检测中，我们已知T^old_new
            ...
            */
            ...
            //(cxn) cur_t cur_r 是优化后的 T^w_i,
            //      vio_t vio_r 是优化前的 T^w2_i
            cur_kf->getPose(cur_t, cur_r);
            cur_kf->getVioPose(vio_t, vio_r);
            m_drift.lock();
            //(cxn) R(yaw_drift)是vio坐标系转到cur坐标系的旋转
            ...
            //(cxn)图优化阶段，可能会有新的帧进来，这里直接根据优化的结果修正他们的pose
            it++;
            for (; it != keyframelist.end(); it++)
            {
                Vector3d P;
